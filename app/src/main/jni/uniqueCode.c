...
int getEncryptCode(unsigned char *encrypt_code) {
    ...
    //printf("length: %zd\n", strlen(plainText));
    int encrypted_length = public_encrypt(plainText, strlen(plainText), publicKey, encrypt_code);
...
int decryptCode(unsigned char *decrypt_code, unsigned char *encrypt_code) {
    ...
    int decrypted_length = private_decrypt(encrypt_code, 256, privateKey, decrypt_code);
